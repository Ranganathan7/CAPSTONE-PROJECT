...
public:
    ...
    vector<int> availability; //robot constraint
    ...
    //gets the input from user and generates a precedence graph
       ...
       file.close();
    	...
    	availability.push_back(2); // 2 number of robot A
    	availability.push_back(1); // 1 number of robot B
    ...
    void robot_constraint(vector<int> &arr){
    	int max_allowed_robots = 0;
		...
		if(max_allowed_robots>=0) return;
		else{
			max_allowed_robots = -1*max_allowed_robots;
			unordered_set<int> temp;
			for(int i=0;i<max_allowed_robots;i++){
				int random = rand()%no_of_stations;
				while(true){
					if(temp.find(random)==temp.end() && arr[random]!=3){
						arr[random] = 3;
						temp.insert(random);
						break;
					}
					random = rand()%no_of_stations;
				}
			}
		}
		return;
    ...
    vector<int> normalise_process_alternative(vector<int> process_alternative){
    	vector<int> ans;
    	for(int i=0;i<no_of_stations;i++){
    		if(process_alternative[i]<=1 || process_alternative[0]>3){
    			int random = 2 + rand()%2;
				ans.push_back(random);
			}
			else{
...
int main()
{
   srand(time(0));	
   int t = 1;
   string s = "industry_data_23_5.txt";
   AOA_ABC object;
   ...
   double ct_avg = 0, energy_avg = 0;
   int n = 0;
   for(int i=0;i<object.EA_seq.size();i++){
	    n++;
	    ct_avg+=(object.EA_obj_values[i][0]);
	    energy_avg+=(object.EA_obj_values[i][1]);
	}
	ct_avg = ct_avg/n;
	energy_avg = energy_avg/n;
	cout<<endl<<endl<<endl;
	cout<<"Cycle time avg : "<<ct_avg<<endl;
	cout<<"Energy avg : "<<energy_avg;
   return 0;
}
